...
local X_BUTTON    = 0x4000  -- XInput X button flag
...
-------------------------------------------------------------
-- XINPUT
-- Inject X button when swing detected
-------------------------------------------------------------
callbacks.on_xinput_get_state(function(retval, user_index, state)
    if state == nil then return end

    if swing_candidate then
        -- Fire X button for one poll
        state.Gamepad.wButtons = state.Gamepad.wButtons | X_BUTTON
