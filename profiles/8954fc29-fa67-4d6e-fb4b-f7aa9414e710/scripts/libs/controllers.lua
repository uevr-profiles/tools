...
local function createControllerComponent(parentActor, sourceName, handIndex)	
	...
	print("Creating controller: " .. sourceNameStr .. " " .. handIndexStr) -- thanks to Lukasblaster
	if parentActor ~= nil then
		...
		print("Couldn't create controller because parentActor was nil")
	end
	return nil
end
...
function M.attachComponentToCachedController(controller, childComponent, socketName, attachType, weld, createIfNotExists)
	if socketName == nil then socketName = "" end
	if attachType == nil then attachType = 0 end
	if weld == nil then weld = false end
	if childComponent ~= nil then
