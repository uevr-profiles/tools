...
function M.handleInput(state, isHoldingWeapon, hand, overrideTrigger)
	...
	for id, target in pairs(inputHandlerAnimID) do
		...
		if not isHoldingWeapon then
			...
			animation.resetAnimation(weaponHandStr.."_"..target, weaponHandStr.."_grip_weapon", false)
			animation.updateAnimation(weaponHandStr.."_"..target, weaponHandStr.."_grip", false --[[uevrUtils.isButtonPressed(state, isRightHanded and XINPUT_GAMEPAD_RIGHT_SHOULDER or XINPUT_GAMEPAD_LEFT_SHOULDER)]], {duration=animDuration})
			...
			if (isRightHanded or overrideTrigger) then --and state.Gamepad.bLeftTrigger > 100 or state.Gamepad.bRightTrigger > 100 then
