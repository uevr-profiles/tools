...
local SparkTemp1= find_required_object("ParticleSystem /Game/Packs/BallisticsVFX/Particles/Impacts/LegacyFX/Small-Medium-Large/Metal/Metal_impact_small.Metal_impact_small")
--Sounds
...
local TempEmitter4=nil
local function SpawnEmitter(dpawn, dworld)
	if dpawn == nil then return end
		TempEmitter1=GameplayStDef:SpawnEmitterAtLocation(dworld,EmTempl,TempVec2,dpawn:K2_GetActorRotation(),TempVec3,false,2,true)
		TempEmitter1:SetActive(false,false)
		--TempEmitter1:Deactivate()
		TempEmitter2=GameplayStDef:SpawnEmitterAtLocation(dworld,EmTempl,TempVec2,dpawn:K2_GetActorRotation(),TempVec3,false,2,true)
		TempEmitter2:SetActive(false,false)
		--TempEmitter2:Deactivate()
		TempEmitter3=GameplayStDef:SpawnEmitterAtLocation(dworld,EmTempl,TempVec2,dpawn:K2_GetActorRotation(),TempVec3,false,2,true)
		TempEmitter3:SetActive(false,false)
		TempEmitter4=GameplayStDef:SpawnEmitterAtLocation(dworld,SparkTemp1,TempVec2,dpawn:K2_GetActorRotation(),TempVec3,false,2,true)
		TempEmitter4:SetActive(false,false)
...
local function SpawnSparks(TargetActor)
	
		TempEmitter4:K2_SetWorldLocation(TargetActor:K2_GetActorLocation(),false,reusable_hit_result,true)
		TempEmitter4:SetActive(true,true)
		--TempEmitter1:Activate()
		TempEmitTimer4=0
		Temp4Active=true
...
local function UpdateEMitterTimer(delta)
	...
	if Temp4Active then
		TempEmitTimer4=TempEmitTimer4+delta
	end
	...
	if TempEmitTimer4>1.5 then
		Temp3Active=false
		TempEmitter4:SetActive(false,false)
		TempEmitter4:Deactivate()
	end
end
...
local function PlaySwingSound(dpawn,dworld)
	if dpawn==nil then return end
	--print(CanSwingR)
	if not dpawn.SpawnInvincibility then
		if PosDiffSecondaryHand>50 and CanSwingL then
			CanSwingL=false
			GameplayStDef:PlaySoundAtLocation(dworld,GetRandomArrayElement(SoundFileSwingsArray),dpawn:K2_GetActorLocation(),dpawn:K2_GetActorRotation(),3,2,0,AttenuationSettingPlayer,SounDConcurrency,dpawn)
		end
		if PosDiffWeaponHand>50 and CanSwingR then
			...
			if dpawn.SecondaryWeapon~=nil then
				...
				elseif  not dpawn.PrimaryEquipped and string.find(dpawn.SecondaryWeapon:get_full_name() , "Katana") then
					GameplayStDef:PlaySoundAtLocation(dworld,SoundFileKatanaUnsheth,dpawn:K2_GetActorLocation(),dpawn:K2_GetActorRotation(),3,2,0,AttenuationSetting,SounDConcurrency,dpawn)
					GameplayStDef:PlaySoundAtLocation(dworld,SoundFileKatanaSwing,dpawn:K2_GetActorLocation(),dpawn:K2_GetActorRotation(),2.5,2,0,AttenuationSettingPlayer,SounDConcurrency,dpawn)
				end
			else GameplayStDef:PlaySoundAtLocation(dworld,GetRandomArrayElement(SoundFileSwingsArray),dpawn:K2_GetActorLocation(),dpawn:K2_GetActorRotation(),3,2,0,AttenuationSettingPlayer,SounDConcurrency,dpawn)
			end
		end
	...
	if not RWeaponHolstered then
		...
		RWeaponMesh:GetOverlappingComponents(_CurrOverlapsRW) 
			...
			for i, comp in ipairs(_CurrOverlapsRW) do
					...
					if Enemy.Health>0 and CanHitRW and  PosDiffWeaponHand>20 and not IsGrabbingR  then
						...
						--if Enemy.Health>0 then
							...
							if Dpawn.SecondaryWeapon~=nil then
								...
								Dpawn.SecondaryWeapon:WeaponHitEnemy(Enemy,true)
				...
				elseif string.find(comp:GetOwner():get_full_name(),"Bullet") then
					if Dpawn.SecondaryWeapon~=nil and not Dpawn.PrimaryEquipped then
						SpawnSparks(comp:GetOwner())
						if Dpawn.SecondaryWeapon.Durability~=nil then	
							Dpawn.SecondaryWeapon.Durability=Dpawn.SecondaryWeapon.Durability-1
						
							if Dpawn.SecondaryWeapon.Durability <=0 then
								Dpawn:DeleteSecondaryWeapon()
							end
						end
					--	Dpawn.SecondaryWeapon:WeaponHitEnemy(comp:GetOwner():GetOwner(),true)
						--print("Blocked")
					end
					print(comp:GetOwner():GetOwner():get_full_name())
					if comp:GetOwner():GetOwner() ~= Dpawn then
						comp:GetOwner():PrepareForDelete()
					end
				end
			end
...
if pawn~=nil and hmd_component~=nil and right_hand_component~=nil and left_hand_component~=nil then
	UpdLastVector()
end
