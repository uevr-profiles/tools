...
local lastFixedCameraChanged = false
...
local configDefinition = {
	{
		...
		layout = {
			...
			-{
				widgetType = "checkbox",
				id = "enable_stafing",
				label = "Enable strafing (experimental)",
				initialValue = false
			},
			{
...
uevr.sdk.callbacks.on_xinput_get_state(function(retval, user_index, state)
	if (state ~= nil) then
		if normalPlay == true then	
			...
			--[[if configui.getValue("enable_stafing") == true then
		...
		--[[if isSolvingPuzzle() then
			if state.Gamepad.wButtons & XINPUT_GAMEPAD_X ~=0 and state.Gamepad.wButtons & XINPUT_GAMEPAD_Y ~=0 then --toggle 2d
				in2dMode = not in2dMode
			end
			if in2dMode == false then 
		...
		else --not in puzzles - see if need to undo 2d
			if in2dMode == true then
				state.Gamepad.wButtons = state.Gamepad.wButtons | XINPUT_GAMEPAD_X 
				state.Gamepad.wButtons = state.Gamepad.wButtons | XINPUT_GAMEPAD_Y 			
			end
			if state.Gamepad.wButtons & XINPUT_GAMEPAD_X ~=0 and state.Gamepad.wButtons & XINPUT_GAMEPAD_Y ~=0 then --suppress attempts to go to 2d when not in puzzles
				print("disable 2d")
				state.Gamepad.wButtons = state.Gamepad.wButtons - XINPUT_GAMEPAD_X
				state.Gamepad.wButtons = state.Gamepad.wButtons - XINPUT_GAMEPAD_Y
			end
...
function strafe(value)
	...
	if value ~= 0 then
		...
		if pawn ~= nil then	
			...
			pcall(setActorLocaion, newLocation)
			triggerPawnMovement()
		end
	end
end
...
function getCurrentWeapon()
	...
	if pawn ~= nil then
		if pawn.WeaponSystemComponent ~=nil and pawn.WeaponSystemComponent.CurrentWeapon ~=nil then
			...
			elseif string.find(pawn.WeaponSystemComponent.CurrentWeapon:get_full_name(), "AutomaticNailer_C") then
				if pawn.WeaponSystemComponent.CurrentWeapon.StaticMesh ~=nil then --NailGun_C
					currentWeapon1 = pawn.WeaponSystemComponent.CurrentWeapon.StaticMesh
					--attach upgrade mesh
					if pawn.WeaponSystemComponent.CurrentWeapon.BlueprintCreatedComponents ~= nil then
			...
			elseif string.find(pawn.WeaponSystemComponent.CurrentWeapon:get_full_name(), "HandCanon_C") then
				if pawn.WeaponSystemComponent.CurrentWeapon.StaticMesh ~=nil then --NailGun_C
					currentWeapon1 = pawn.WeaponSystemComponent.CurrentWeapon.StaticMesh
					--attach upgrade mesh
					if pawn.WeaponSystemComponent.CurrentWeapon.BlueprintCreatedComponents ~= nil then
...
function updateEquippedWeapon(currentWeapon, hand)
	...
	if currentWeapon ~= nil and activeWeapon ~= currentWeapon then
		...
		elseif string.find(currentWeapon:get_full_name(), "AxeWeapon_C") then
			state:set_rotation_offset(Vector3f.new(0, -1.751, 0))
			state:set_location_offset(Vector3f.new(4.040, 0.910, -2.940)) --forward , up, right
...
function applySClimbingSettings()
	if lastFixedCameraChanged == false then
		applyLastFixedCamera()
		lastFixedCameraChanged = true
	end
	normalPlay = false
	--vr.set_mod_value("VR_2DScreenMode", true)
