...
local OffsetLocAugY = -3 
local OffsetLocAugZ = 6 
local OffsetLocTrijY = -3
local OffsetLocTrijZ = 6 
...
local function GetComponentOffset(weapon_mesh)
	...
	--if scope_plane_component~=nil then
		if weapon_mesh~=nil then
			...
			elseif string.find(ScopeType, "P_PS") then
				ZOffset  = 9.5
				YOffset  = -13.5	
				XOffset =1.5
				Scale = 0.038
...
local function attach_components_to_weapon(weapon_mesh)
	...
	--if not string.find(api:get_local_pawn(0).m_currentEquipment:get_fname():to_string(),"Bino") and not string.find(Get_Scope_Object(api:get_local_pawn(0):GetCurrentArm()):get_fname():to_string(),"Collimator")  then
		...
		scope_mesh = get_scope_mesh(weapon_mesh)
			...
			scene_capture_component:K2_SetRelativeLocation(temp_vec3:set(0, 60,0), false, reusable_hit_result, false)
			scene_capture_component:SetVisibility(false)
		end
		...
		scope_mesh = get_scope_mesh(weapon_mesh)--get_scope_mesh(weapon_mesh)
			...
			scope_plane_component:K2_SetRelativeLocation(temp_vec3:set(GetComponentOffset(weapon_mesh).x, GetComponentOffset(weapon_mesh).y -0.01,GetComponentOffset(weapon_mesh).z ), false, reusable_hit_result, false)--temp_vec3:set(5.310, -21.470,4.860 ), false, reusable_hit_result, false)
			scope_plane_component:SetWorldScale3D(temp_vec3:set(GetComponentOffset(weapon_mesh).Scale,GetComponentOffset(weapon_mesh).Scale,0.01))
		...
		scope_mesh = get_scope_mesh(weapon_mesh)--get_scope_mesh(weapon_mesh)
			...
			black_back_component:K2_SetRelativeLocation(temp_vec3:set(GetComponentOffset(weapon_mesh).x, GetComponentOffset(weapon_mesh).y,GetComponentOffset(weapon_mesh).z ), false, reusable_hit_result, false)--temp_vec3:set(5.310, -21,4.860  ), false, reusable_hit_result, false)
			black_back_component:SetWorldScale3D(temp_vec3:set(GetComponentOffset(weapon_mesh).Scale,GetComponentOffset(weapon_mesh).Scale, 0.01))
		...
		if reticle_plane_component then
			...
			reticle_plane_component:K2_SetRelativeLocation(temp_vec3:set(GetComponentOffset(weapon_mesh).x, GetComponentOffset(weapon_mesh).y-0.02,GetComponentOffset(weapon_mesh).z  ), false, reusable_hit_result, false)
...
local SwitchDelta=0
local isSwitchedWpn=false
local RefreshScope=false
local function StartDelta(dDelta)
	...
	if SwitchDelta >2.5 then
		...
		RefreshScope=true
		--print(SwitchDelta)
		 --resets request from collisionb lua
	end
end	
		
local PreUnequipWpn=nil
local WpnChangeDelta=0
       ...
       if viewport then
		...
		if weapon_Obj ~= nil  then
			...
			--if SwitchDelta>2.5 then
				--isSwitchedWpn=false
			--end
		elseif not isSwitchedWpn then
			--print("empty Hands") 
			isSwitchedWpn =true
		end
		
		if isSwitchedWpn then
			StartDelta(delta)
		end	
        ...
        if weapon_mesh  then
			...
			--	end)
				...
				if scope_plane_component == nil and black_back_component==nil and scene_capture_component==nil and reticle_plane_component==nil then
		...
		--destroy_actor(scope_actor)
			...
			if UEVR_UObjectHook.exists(scope_plane_component) then
				scope_plane_component  :SetVisibility(false)
