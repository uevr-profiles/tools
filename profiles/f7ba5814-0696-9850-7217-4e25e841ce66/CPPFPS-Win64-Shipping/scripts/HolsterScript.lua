...
--			unpressButton(state, XINPUT_GAMEPAD_LEFT_THUMB)
	...
	if isRhand then	
		if  RZone ~=0 and inMenu==false  then
			--unpressButton(state, XINPUT_GAMEPAD_LEFT_SHOULDER)
			unpressButton(state, XINPUT_GAMEPAD_RIGHT_SHOULDER)
			--unpressButton(state, XINPUT_GAMEPAD_LEFT_THUMB)
			unpressButton(state, XINPUT_GAMEPAD_RIGHT_THUMB)
		...
		if LZone ~= 0 and inMenu==false  then
			--unpressButton(state, XINPUT_GAMEPAD_LEFT_SHOULDER)
			unpressButton(state, XINPUT_GAMEPAD_RIGHT_SHOULDER)
			--unpressButton(state, XINPUT_GAMEPAD_LEFT_THUMB)
			unpressButton(state, XINPUT_GAMEPAD_RIGHT_THUMB)
	...
	if RWeaponZone == 2 and inMenu==false  then
		state.Gamepad.bLeftTrigger=0
	end
	if RWeaponZone== 1 and inMenu==false  then
		unpressButton(state, XINPUT_GAMEPAD_LEFT_SHOULDER)
	end
	...
	--Inputchanges
	if math.abs(ThumbRY)< 200 and isJump==1 then
		
		pawn:BPInputJumpReleased()
		isJump=0
	end
	if math.abs(ThumbRY)< 200 and isCrouched then
		isCrouched=false
		pawn:BPInputCrouchReleased()
	...
	if ThumbRY <-30000 and isCrouched==false and inMenu==false then
		if canSlide ==false then
		pawn:BPInputCrouchPressed()
		isCrouched=true
		else 
		player= api:get_player_controller(0)
		player:InpActEvt_Slide_K2Node_InputActionEvent_5(Key)
		isSliding=true
		isCrouched=true
		end
	end
	if ThumbRY > 30000 and isJump ==0 and inMenu==false  then
		pawn:BPInputJumpPressed()
		isJump=1
	end
			--switch sprint 
	if lShoulder and RWeaponZone ~= 1 and inMenu==false  then
		if isCrouched  then
			isCrouched=false
			pawn:BPInputCrouchReleased()
		end
		unpressButton(state, XINPUT_GAMEPAD_LEFT_SHOULDER)
		--pressButton(state, XINPUT_GAMEPAD_LEFT_THUMB)
		pawn["WalkSpeed"] = 800.000
		canSlide=true
	else
		pawn["WalkSpeed"] = 400.000
		canSlide=false		
	end
	if Abutton and inMenu==false then
		unpressButton(state, XINPUT_GAMEPAD_A)
		pressButton(state, XINPUT_GAMEPAD_RIGHT_SHOULDER)
	end
	--if Xbutton and inMenu==false then
	--	unpressButton(state,XINPUT_GAMEPAD_X)
	--	pressButton(state,XINPUT_GAMEPAD_A)
	--end
	if LTrigger>0 then
		state.Gamepad.bLeftTrigger=0
		if LTrigger>200 then
			pressButton(state, XINPUT_GAMEPAD_LEFT_SHOULDER)
		end
	end
...
--	 ActiveWeapons = pawn.ActiveWeapons
--
--	 WeaponSlots = ActiveWeapons["Element 0"]
--	
--	 Element = WeaponSlots.Test
--	
--	print(Element)--:get_fname():to_string())
	--MouseCursor
	if api:get_player_controller().bShowMouseCursor then
	...
	--inmenu??
	--if not lGrabActive and not rGrabActive then
	--inMenu = api
	--end
	--GrenadeReady
	if GrenadeReady and rGrabActive== false then
		pawn:BPInputThrowGrenade()
		GrenadeReady=false
	end
	--flashlight? to ambient light
	if pawn.Flashlight~=nil then
		if pawn.Flashlight.bVisible then
			pawn.AmbientLight:SetVisibility(true)
		else  pawn.AmbientLight:SetVisibility(false)
		end
	end
	...
	--		SendKeyUp('I')
	--	end
	--	if KeyGReady==true then
	--		SendKeyDown('G')
	--		KeyGReady=false
	--	end
	--	if KeyGReady ==false and KeyG==true then
	--		SendKeyUp('G')
	--		KeyG=false
	--	end
	--	if KeyM==true then
