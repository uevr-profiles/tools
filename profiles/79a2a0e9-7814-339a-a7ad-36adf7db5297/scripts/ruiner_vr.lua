...
local onReceiveActorEndOverlapHooked
...
local lastPawnRotationIn1stPersonYaw = nil
...
uevr.sdk.callbacks.on_post_engine_tick(function(engine, delta)
	...
	if inRegularPlay == true and isRegularPlay() == true then		
		...
		--triggerNPCInteraction1()
	end
end)

function getCurrentWeaponMesh()
	local pawn = api:get_local_pawn(0)
	if pawn ~= nil and pawn.EquippedWeapon ~= nil and pawn.EquippedWeapon.Mesh ~= nil and not string.find(pawn.EquippedWeapon.Mesh:get_full_name(), "Player_Pipe_C") 
		and not string.find(pawn.EquippedWeapon.Mesh:get_full_name(), "Katana_C")
		and not string.find(pawn.EquippedWeapon.Mesh:get_full_name(), "KATANA_C")
		and not string.find(pawn.EquippedWeapon.Mesh:get_full_name(), "AXE_C")
		and not string.find(pawn.EquippedWeapon.Mesh:get_full_name(), "AXE_C")
		and not string.find(pawn.EquippedWeapon.Mesh:get_full_name(), "Cyborg_Fist_C")
		and not string.find(pawn.EquippedWeapon.Mesh:get_full_name(), "Rumble_C") then
...
uevr.sdk.callbacks.on_pre_engine_tick(function(engine, delta)
	...
	inRegularPlay = false
	if isMainMenu() then
		lastPawnRotationIn1stPersonYaw = nil
		applyMainMenuSettings()
	elseif isShouldApplySecurityLockYeetFix(delta) then
		if lastPawnRotationIn1stPersonYaw == nil then
			local pawn = api:get_local_pawn(0)
			if pawn ~= nil then
				lastPawnRotationIn1stPersonYaw = pawn:K2_GetActorRotation().Yaw
			end
		end
		--revertDirection()
	...
	elseif isInCutScene() then
		...
		if lastPawnRotationIn1stPersonYaw ~=nil then
			...
			if pawn ~= nil then
				...
				newRotation.Yaw = lastPawnRotationIn1stPersonYaw
				lastPawnRotationIn1stPersonYaw = nil
				pcall(setActorRotation, newRotation)
				print("reverted yaw to last value")
			end
		end
...
function hookFuntionsForRuntimeGeneratedClasses() 
	...
	--[[if onReceiveActorEndOverlapHooked == false then
		hook_function("BlueprintGeneratedClass /Game/Blueprints/AI/Characters/AICharacter_Base.AICharacter_Base_C", "ReceiveActorEndOverlap", false,
			function(fn, obj, locals, result)
				print("ReceiveActorEndOverlap started")
				return false
			end,
			function(fn, obj, locals, result)
				print("ReceiveActorEndOverlap ended")
				return false
			end,
		true)
		
		onReceiveActorEndOverlapHooked = true
	end]]
end
...
function triggerNPCInteraction1()
	local allNPCActions = uevrUtils.find_all_instances("BlueprintGeneratedClass /Game/Blueprints/Custom_Actions/SplineTeleport_NPC.SplineTeleport_NPC_C", true)
	if allNPCActions ~= nil then
		for _, npcAction in pairs(allNPCActions) do
			--npcAction.bActorEnableCollision = false
			--npcAction.bEnableAutoLODGeneration = true
			--npcAction:ReceiveActorBeginOverlap(npcAction)
			if npcAction.BlueprintCreatedComponents ~= nil then
				for _, component in pairs(npcAction.BlueprintCreatedComponents) do
					if string.find(component:get_full_name(), "TransitionTrigger") then
						print ("working on transitionTrigger", component:get_full_name())
						--component:ReceiveBeginPlay()
						--component:ReceiveTick(0.0001)
						--component.DetailMode = 1
					end
				end
			end
		end
...
function triggerNPCInteraction()
	local pawn = api:get_local_pawn(0)
	if pawn ~= nil then
	
		local allNPCs = uevrUtils.find_all_instances("BlueprintGeneratedClass /Game/Story/Characters/AICharacter_Civilian_Female_01.AICharacter_Civilian_Female_01_C", false)
		if allNPCs ~= nil then
			for _, npc in pairs(allNPCs) do
				if npc ~= nil then
					if npc.SpawnDist ~= nil then
						npc.SpawnDist = 99999.0
					end
					if npc.SpawnDistSquared ~= nil then
						npc.SpawnDistSquared = 99999.0 * 99999.0
					end
					npc.Mesh:SetVisibility(true)
					if npc.HeadMesh ~= nil then
						npc.HeadMesh:SetVisibility(true)	
						npc.HeadMesh.bComponentUseFixedSkelBounds = true
						npc.HeadMesh.bEnableUpdateRateOptimizations = false
						npc.HeadMesh.bHiddenInGame = false
						npc.HeadMesh:SetRenderInMainPass(true)
						npc.HeadMesh:SetRenderCustomDepth(true)
					end
					if npc.HairMesh ~= nil then
						npc.HairMesh:SetVisibility(true)
						npc.HairMesh.bComponentUseFixedSkelBounds = true
						npc.HairMesh.bEnableUpdateRateOptimizations = false
						npc.HairMesh.bHiddenInGame = false
						npc.HairMesh:SetRenderInMainPass(true)
						npc.HairMesh:SetRenderCustomDepth(true)						
					end
					if npc.LegsMesh ~= nil then
						npc.LegsMesh:SetVisibility(true)
						npc.LegsMesh.bComponentUseFixedSkelBounds = true
						npc.LegsMesh.bEnableUpdateRateOptimizations = false
						npc.LegsMesh.bHiddenInGame = false
						npc.LegsMesh:SetRenderInMainPass(true)
						npc.LegsMesh:SetRenderCustomDepth(true)	
					end
				end
			end
		end
		allNPCs = uevrUtils.find_all_instances("BlueprintGeneratedClass /Game/Story/Characters/AICharacter_Civilian_Male_01.AICharacter_Civilian_Male_01_C", false)
		if allNPCs ~= nil then
			for _, npc in pairs(allNPCs) do
				if npc ~= nil then
					if npc.SpawnDist ~= nil then
		...
		allNPCs = uevrUtils.find_all_instances("BlueprintGeneratedClass /Game/Blueprints/AI/Characters/AICharacter_Base.AICharacter_Base_C", true)
		if allNPCs ~= nil then
			for _, npc in pairs(allNPCs) do
				if npc ~= nil then
					if npc.SpawnDist ~= nil then
					...
					if npc.CheckSpawnTimer ~= nil then
						npc.CheckSpawnTimer.Handle = 1
					end
					if npc.SpawnCollisionHandlingMethod ~= nil then
						npc.SpawnCollisionHandlingMethod = 2
					end
					if npc.bCollideWhenPlacing ~= nil then
						npc.bCollideWhenPlacing  = false
					end
					 
					
					
					--print ("working on npc", npc:get_full_name())
					--npc:ApplyFire()
					--npc:ReceiveTick(1)
					if npc.Mesh.AnimScriptInstance ~= nil then
						npc.Mesh.AnimScriptInstance:BlueprintInitializeAnimation()
					end
					--npc.Mesh.bComponentUseFixedSkelBounds = true
					--npc.Mesh.bEnableUpdateRateOptimizations = false
					--npc.Mesh.bHiddenInGame = false
					--npc.Mesh:SetRenderInMainPass(true)
					--npc.Mesh:SetRenderCustomDepth(true)
					npc.Mesh:SetVisibility(true)
					if npc.HeadMesh ~= nil then
						npc.HeadMesh:SetVisibility(true)	
						npc.HeadMesh.bComponentUseFixedSkelBounds = true
					...
					npc:ReceiveActorBeginOverlap(npc)
					--npc.Mesh:SetVisibility(false)
...
function fixDirection()
	...
	if pawn ~= nil then
		...
		if currentCameraSpringArm ~= nil then
			...
			currentCameraSpringArm.TargetArmLength = 0
	...
	if (state ~= nil) then
		...
		if normalPlay == true then
			if state.Gamepad.wButtons & XINPUT_GAMEPAD_Y ~= 0 then -- abilities wheel fix - allow both sticks to select abilities while holding Y
				if state.Gamepad.sThumbLX < 1000 and state.Gamepad.sThumbLY < 1000 and state.Gamepad.sThumbLX > -1000 and state.Gamepad.sThumbLY > -1000 then 
