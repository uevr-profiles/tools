...
local engine = game_engine_class:get_first_object_matching(false)
	...
	if CurrCam~=testCam and CurrCam~=nil then
		SwappedCam=CurrCam
	...
	if pawn~=nil then
		...
		if controllers.getController(2)~=nil then
			HmdVector=controllers.getController(2):GetForwardVector()
		end
		...
		if pawn~=nil and testCam~=nil  then
			...
			AimVector=KismetMathLibrary:Add_VectorVector(pawn:K2_GetActorLocation(),HmdVector)
		...
		if controllers.getController(1)~=nil then
			HandVector= controllers.getController(1):GetForwardVector()
		end
		--print(HmdVector.Y)
