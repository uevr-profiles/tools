...
local function RetRem()
	...
	if ret_mesh ~= nil and ret_mesh.Image_DownLaser ~= nil and is_crosshair_visible == true then
...
local function ResetPlayUI()
	...
	if configui.getValue("ui_follows_head") then
		...
		params.vr.set_mod_value("UI_Distance", configui.getValue("ui_position").Z)
		params.vr.set_mod_value("UI_Size", "0.50")
		params.vr.set_mod_value("UI_X_Offset", configui.getValue("ui_position").X)
		params.vr.set_mod_value("UI_Y_Offset", configui.getValue("ui_position").Y)
	else
		params.vr.set_mod_value("VR_DecoupledPitchUIAdjust", "true")
...
uevr.sdk.callbacks.on_pre_engine_tick(function(engine, delta)
			...
			if not string.find(active_weap:get_full_name(), "NoWeapon") then
				...
				if cur_weap == last_weapon then 
					...
					if u_exist == true then
						if configui.getValue("show_hands") == false then
	...
	if pawn == nil or string.find(pawn:get_full_name(), "Starting") or seq_active == true or is_mouse == true or eval_active == true or is_hidden == true or is_load == false or detective == true or is_input == false or is_view == false or is_menu == true or is_tut == true then
		...
		if Playing == false then
			...
			UEVR_UObjectHook.set_disabled(false)
			params.vr.set_aim_method(configui.getValue("left_hand_mode") and 3 or 2)
		end
		...
		if string.find(pawn:get_full_name(), "L15_ShadyMeeting") then
			...
			shady_active = false
			if not configui.getValue("useReticule") then
