...
local BButtonPreviouslyPressed = false
...
-- Detect current BButton pressed state
-- These values can be changed to map control to a different XInput button
-- Button  XInput Value
-- A	   0x1000
-- B	   0x2000
-- X	   0x4000
...
local BButtonPressed = (state.Gamepad.wButtons & XINPUT_GAMEPAD_B) ~= 0
  
  -- On press down (transition)
  if BButtonPressed and not BButtonPreviouslyPressed then
    ...
    params.functions.log_info("[HelperScript] BButton pressed: applied VR params toggle")
    params.functions.log_info("[HelperScript] Toggle cutscene lock by long press")

  elseif not BButtonPressed and BButtonPreviouslyPressed then
    ...
    if backupParams.aim_method ~= nil then
      ...
      params.functions.log_info("[HelperScript] BButton released: restored VR params")
