	...
	if configuration ~= nil and configuration["profiles"] ~= nil then
		...
		if profile ~= nil then
			...
			--if M.exists() then
				...
				accessories.setAnimationIDs(configuration["attachments"])
...
local function resolveAccessoryMarkerParamsForAttach(accessoryParams, useMontageProximity, animInstance, montageObject, markerIndexOverride, strictMontageTime)
	...
	if useMontageProximity then
		if animInstance == nil or montageObject == nil or animInstance.Montage_GetPosition == nil then
			...
			if ok then
				local markerParams, markerIndex = accessories.resolveMarkerParamsForTime(accessoryParams, currentTime)
	...
	local markerParams = accessories.getPrimaryMarkerParams(accessoryParams)
...
local function proximityAccessoryForHand(hand)
    ...
    local list = accessories.getAccessoriesForAttachment(attachmentID)
    ...
    for accessoryID, accessoryParams in pairs(list) do
		local markerParams = accessories.getPrimaryMarkerParams(accessoryParams) or accessoryParams
...
function M.attachHandToController(hand)
	for name, components in pairs(handComponents) do
		...
		if uevrUtils.getValid(component) ~= nil then
			...
			local rot = uevrUtils.rotator(0, 0, 0)
			local loc = uevrUtils.vector(0, 0, 0)
			local scale = uevrUtils.vector(1, 1, 1)
			local hitResult = uevrUtils.get_struct_object("ScriptStruct /Script/Engine.HitResult")
			component:K2_SetRelativeRotation(rot, false, hitResult, false)
