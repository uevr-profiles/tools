	...
	if PosDiffWeaponHand>50 and CanSwingR then
		...
		if dpawn.SecondaryWeapon~=nil then
			if not string.find(dpawn.SecondaryWeapon:get_full_name() , "Katana") then
				...
				GameplayStDef:PlaySoundAtLocation(world,SoundFileKatanaUnsheth,dpawn:K2_GetActorLocation(),dpawn:K2_GetActorRotation(),4,2,0,AttenuationSetting,SounDConcurrency,dpawn)
					...
					if Enemy.Health>0 and CanHitRW and  PosDiffWeaponHand>20 and not IsGrabbingR  then
						...
						--if Enemy.Health>0 then
							...
							if Dpawn.SecondaryWeapon~=nil then
								...
								if string.find(Dpawn.SecondaryWeapon:get_full_name() , "Katana") then
