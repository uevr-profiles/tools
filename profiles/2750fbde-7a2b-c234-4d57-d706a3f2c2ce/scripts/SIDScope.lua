...
local emissive_material_amplifier = 1.5
local fov = 18.0

-- Static variables
local emissive_mesh_material_name = "Material /Engine/EngineMaterials/EmissiveMeshMaterial.EmissiveMeshMaterial"
local BlackBackMaterial_name= 		"Material /Engine/BasicShapes/BasicShapeMaterial.BasicShapeMaterial"
...
local sightTexture_name = "Texture2D /Game/Weapon/Rifle_M16A4/TX_M16A4_Reticle_Alpha.TX_M16A4_Reticle_Alpha"
...
local function init_static_objects()
	...
	sightTexture_name = "Texture2D /Game/Weapon/Rifle_M16A4/TX_M16A4_Reticle_Alpha.TX_M16A4_Reticle_Alpha"
...
local function attach_components_to_weapon(weapon_mesh)
	...
	--if not string.find(api:get_local_pawn(0).m_currentEquipment:get_fname():to_string(),"Bino") and not string.find(Get_Scope_Object(api:get_local_pawn(0):GetCurrentArm()):get_fname():to_string(),"Collimator")  then
		...
		scope_mesh = get_scope_mesh(weapon_mesh)
			...
			scene_capture_component:K2_SetRelativeRotation(temp_vec3:set(90, 0, 0), false, reusable_hit_result, false)
			scene_capture_component:K2_SetRelativeLocation(temp_vec3:set(0, -19, 20), false, reusable_hit_result, false)
		...
		scope_mesh = get_scope_mesh(weapon_mesh)--get_scope_mesh(weapon_mesh)
			...
			scope_plane_component:K2_SetRelativeRotation(temp_vec3:set(180, 0, 0), false, reusable_hit_result, false)
			scope_plane_component:K2_SetRelativeLocation(temp_vec3:set(-0, -17.5,-0.2 ), false, reusable_hit_result, false)
		...
		scope_mesh = get_scope_mesh(weapon_mesh)--get_scope_mesh(weapon_mesh)
			...
			black_back_component:K2_SetRelativeRotation(temp_vec3:set(180, 0, 0), false, reusable_hit_result, false)
			black_back_component:K2_SetRelativeLocation(temp_vec3:set(-0, -17.5,0 ), false, reusable_hit_result, false)
			black_back_component:SetWorldScale3D(temp_vec3:set(0.03,0.03, 0.00001))
		...
		if reticle_plane_component then
			...
			reticle_plane_component:K2_SetRelativeRotation(temp_vec3:set(180, 90, 0), false, reusable_hit_result, false)
			reticle_plane_component:K2_SetRelativeLocation(temp_vec3:set(-0, -17.4,-0.21 ), false, reusable_hit_result, false)
			reticle_plane_component:SetWorldScale3D(temp_vec3:set(0.07,0.02, 0.00001))
...
uevr.sdk.callbacks.on_pre_engine_tick(
	function(engine, delta)
		...
		weapon_mesh=nil
        weapon_Obj = c_pawn.Inventory.ActiveWeapon --:GetCurrentArm()--get_equipped_weapon(c_pawn)
		
		
		if weapon_Obj ~= nil then
			--weapon_mesh=weapon_Obj.m_meshFirstPerson
			weapon_mesh=weapon_Obj.PickupMesh
		end
	--	pcall(function()
