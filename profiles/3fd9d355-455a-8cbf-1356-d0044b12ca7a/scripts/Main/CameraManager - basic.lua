...
if pawn~=nil then
	Cam= player.PlayerCameraManager.ActiveCameraState.CameraComp
	Cam:SetFieldOfView(90)
	Cam:K2_AttachToComponent(right_hand_component,"",0,0,0,true)
...
--	if hmd_component~=nil then	
	...
	local altRot= Cam:K2_GetComponentRotation()	
	...
	--	TargetObjLoc=CurrentWeaponMesh:K2_GetComponentLocation()
	--end
...
--    UEVR_UObjectHook.get_or_add_motion_controller_state(Mesh):set_permanent(true)
		...
		--local hmd_componenttest= controllers.getController(2)
		if hmd_component	~=nil then	
			--print("HmdFound")
...
--Read Gamepad stick input for rotation compensation
	...
	if  HeadBasedMovement then-- and not isCinematic and not isMenu then
...
local pawn = api:get_local_pawn(0)	
if pawn~=nil and not isCinematic and not isMenu then
