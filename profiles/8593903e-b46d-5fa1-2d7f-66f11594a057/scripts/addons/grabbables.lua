...
local grabbedMapping = {
	...
	{mesh="SM_Barbells_01", pos={0.6, 0.2, -0.8}, rot={0.0, -14.6, 90}},
	{mesh="SM_Barbells_02", pos={0.6, 0.2, -0.8}, rot={0.0, -14.6, 90}},
	{mesh="SM_Chair_01a", pos={-29.6, 16.0, 30.2}, rot={14.4, -0.2, 9.4}},
	{mesh="SM_LP_Radiator1", pos={7.8, 9.6, -22.6}, rot={0, 0, 0}},
}
	...
	if uevrUtils.getValid(weapon) ~= nil and (offsetType == 1 or offsetType == 2) then
		...
		for i = 1, #grabbedMapping do
			...
			if name ~= nil then
				...
				local mesh = grabbedMapping[i]["mesh"]
				if mesh ~= nil and weapon.StaticMesh ~= nil then
					local staticMeshName = weapon.StaticMesh:get_full_name()
					if string.find(staticMeshName, mesh) then
						local value = uevrUtils.vector(grabbedMapping[i][ext[offsetType]])
						M.print("Found offset for " .. weaponName .. " " .. offsetType .. " " .. value.X .. " " .. value.Y .. " " .. value.Z )
						if value ~= nil then
							return value
