...
local configDefinition = {
	{
		...
		saveFile = "config_scope",
		isHidden=(currentLogLevel ~= LogLevel.Debug),
		layout = 
		{		
			{
				widgetType = "checkbox",
...
function M.setScopeBrightness(value)
...
function M.createOcularLens(renderTarget2D, options)
	...
	if uevrUtils.getValid(scopeMeshComponent) ~= nil then 
		...
		if templateMaterial ~= nil then
			...
			M.setScopeBrightness(options.brightness)
		end
